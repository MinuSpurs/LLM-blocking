...
def collect_unique_documents(query, file_path, json_num, batch_size=10, max_disp_len=200, target_count=10):
    ...
    while len(seen_doc_ix) < target_count:
        ...
        # error checking
        if len(seen_doc_ix) != len(all_documents):
        ...
        if len(seen_doc_ix)>=json_num or len(seen_doc_ix)>=target_count:
            break
...
def main():
    ...
    expressions = {'is an invented word':1977, 'is a made up word':4754}

    for expression in expressions:
        collect_unique_documents(expression, f"./data/{'_'.join(expression.split())}.json", json_num=expressions[expression], batch_size=10, max_disp_len=200, target_count=expressions[expression])

    for expression in expressions:
        file_path = f"./data/{'_'.join(expression.split())}.json"
        with open(file_path, "r") as file:
            data = json.load(file)      
            item_count = len(data)
            print(file_path, '\t', item_count)
